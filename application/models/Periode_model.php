...
class Periode_model extends CI_Model {
    ...
    // Get From Databases
    function get_periode_mahasiswa($params = array())
    {
        if(isset($params['id']))
        {
            $this->db->where('periode.periode_id', $params['id']);
        ...
        {
            $this->db->where('input_transaction.student_student_id', $params['student_id']);
        }
        ...
        $this->db->select('periode.periode_id, periode_date, periode_total_budget, periode_description,
            periode_input_date, periode_last_update');
        $this->db->select('periode.user_user_id, user_name');
        $this->db->join('user', 'user.user_id = periode.user_user_id', 'left');
        $this->db->join('input_transaction', 'input_transaction.periode_periode_id = periode.periode_id', 'left');
        $res = $this->db->get('periode');
